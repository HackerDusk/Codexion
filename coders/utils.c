...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/09/06 02:49:15 by srandro           #+#    #+#             */
/*   Updated: 2026/09/06 02:49:16 by srandro          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_alert(t_monitor *monitor)
{
	...
	while (i < monitor->nb_coders)
	{
		if (monitor->coders[i].compiles_done < monitor->coders[i].number_of_compiles_required)
		{	
			if (curr_time >= (
				monitor->coders[i].last_compile_start +
				monitor->coders[i].time_to_burnout))
