		...
		&& !is_simulation_stopped(coder->monitor))
        {
            if (get_time_ms() < coder->first->available_at)
            {
                ...
                pthread_cond_timedwait(
                    &coder->first->dongle_cond, &coder->first->dongle_mutex, &ts);
            }
            else
                pthread_cond_wait(&coder->first->dongle_cond, &coder->first->dongle_mutex);
        }
...
int continue_after_second_dongle_access(t_coder *coder, struct timespec ts)
{
		...
		&& !is_simulation_stopped(coder->monitor))
    {
        if (get_time_ms() < coder->second->available_at)
        {
            ...
            pthread_cond_wait(&coder->second->dongle_cond, &coder->second->dongle_mutex);
	}
	if (is_simulation_stopped(coder->monitor))
	{
		...
		coder->first->available_at = get_time_ms() + coder->first->dongle_cooldown;
